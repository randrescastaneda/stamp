---
title: "Hashing, Change Detection & Versions"
...
Milestone 2 adds **stable hashing**, **skip-on-equal** saves, and a lightweight **versions** catalog.
...
> Current policy: **content** changes drive new versions. Changing `code=` alone won’t force a new version unless content also changes (we still record the new `code_hash` when we do write). If you want code-only changes to force writes, you can add a package option for that later.
