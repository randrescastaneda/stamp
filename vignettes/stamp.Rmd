...
- Artifact: a saved object on disk (one file path managed by stamp).
- Sidecar: the metadata file that accompanies each artifact (PKs, parents, code hash, domain, timestamps).
- Version: an immutable snapshot of an artifact; each `st_save()` that produces a change creates a new version id.
- Parent: a pointer (path + version_id) recorded in a sidecar that identifies an upstream artifact used to produce a downstream artifact.
- Lineage: the directed graph of artifacts linked by parent relationships; `st_lineage()` inspects this graph.
- Builder: a small function you register with `st_register_builder()` that knows how to produce a specific artifact. A builder typically has the signature `function(path, parents)` and returns a list containing at least `x` (the object to be saved) and `code` (the function(s) used to produce `x`).
- Plan: the result of `st_plan_rebuild()` — a computed list of actions that should be executed because artifacts are stale (parents changed, code changed, or missing).
- Rebuild: executing a plan with `st_rebuild()` — this runs registered builders to produce and save artifacts, updating sidecars and versions.
...
## Appendix: Glossary and Concepts (Artifacts, Builders, Plans, Rebuilds)

This appendix consolidates the conceptual explanation so the main narrative above stays focused on the workflow steps.
...
General idea
1. Register builders for reproducible targets (pure, deterministic, no direct saving inside builder).
2. Compute a plan (`st_plan_rebuild()`) to see what changed and what must be recomputed.
3. Inspect/optionally filter the plan.
4. Run `st_rebuild(plan)` to execute only necessary work.

How the vignette used these ideas
- Registered a builder for `out_summary_path` that returns `foo()` and code metadata.
- Mutated CPI to create a new upstream version; `st_is_stale(out_summary_path)` became TRUE.
- Planned and rebuilt, causing only the summary artifact to be recomputed.
- Later, partition builders were registered for per-key summaries (illustrating fine-grained incremental recomputation).

Practical tips
- Pass multiple producing functions as a small named list: `code = list(foo = foo, helper = f1)` so any helper change invalidates the target.
- Builders should rely on their `parents` argument when possible—this makes them testable and minimizes global reads.
- Keep domains (`domain = "welfare"`, `"summary"`, etc.) consistent; they are lightweight classification tags useful for filtering, retention, or dashboards.
- Use strict mode in `st_plan_rebuild(mode = "strict")` for conservative rebuild decisions; relax if performance dictates.

Example builder pattern (parent-driven, pure):

```{r eval=FALSE}
st_register_builder(out_summary_path, function(path, parents) {
  welfare_parent_paths <- vapply(parents, function(p) grepl("welfare", p$path), logical(1))
  welfare_list <- lapply(parents[welfare_parent_paths], function(p) st_load(p$path))
  cpi_tbl <- st_load(fs::path(macro_dir, "cpi.qs2"))
  gdp_tbl <- st_load(fs::path(macro_dir, "gdp.qs2"))
  pop_tbl <- st_load(fs::path(macro_dir, "population.qs2"))
  out <- bar(welfare_list, cpi_tbl, gdp_tbl, pop_tbl)
  list(x = out, code = list(bar = bar), code_label = "aggregate_welfare_partitioned")
})
plan <- st_plan_rebuild(targets = out_summary_path, include_targets = TRUE, mode = "strict")
st_rebuild(plan)
```

This appendix centralizes the concepts; remove or adapt it if you later create a dedicated vignette just for architecture.
