...
```
    ...
    ## ✔ stamp initialized
    ##   root: /tmp/Rtmppy0xMi/stamp-vignette
    ##   state: /tmp/Rtmppy0xMi/stamp-vignette/.stamp
...
```
    ...
    ## /tmp/Rtmppy0xMi/stamp-vignette/.stamp
    ## ├── logs
    ## └── temp
...
```
    ...
    ## ✔ Saved [qs2] → /tmp/Rtmppy0xMi/stamp-output/example.qs2 @ version
    ##   749e544aad17cf75
...
```
    ...
    ## /tmp/Rtmppy0xMi/stamp-output/example.qs2
...
```
    ...
    ## Warning: No primary key recorded for /tmp/Rtmppy0xMi/stamp-output/example.qs2.
...
```
    ...
    ## List of 11
    ##  $ path        : chr "/tmp/Rtmppy0xMi/stamp-output/example.qs2"
    ##  $ format      : chr "qs2"
    ##  $ created_at  : chr "2025-11-26T15:30:52Z"
    ##  $ size_bytes  : int 256
...
```
    ...
    ## ✔ Saved [qs2] → /tmp/Rtmppy0xMi/stamp-output/upstream.qs @ version
    ##   5756ef5a7cbf322e
...
```
    ...
    ## ✔ Saved [qs2] → /tmp/Rtmppy0xMi/stamp-output/derived.qs @ version
    ##   c77d6b07a6f93833
...
```
    ...
    ## $path
    ## [1] "/tmp/Rtmppy0xMi/stamp-output/derived.qs"
    ## 
    ## $format
    ## [1] "qs2"
    ## 
    ## $created_at
    ## [1] "2025-11-26T15:30:53Z"
    ## 
    ## $size_bytes
    ## [1] 256
    ## 
    ...
    ## $parents
    ##                                       path       version_id
    ## 1 /tmp/Rtmppy0xMi/stamp-output/upstream.qs 5756ef5a7cbf322e
    ## 
    ## $attrs
    ## list()
...
```
    ...
    ## 1     1 /tmp/Rtmppy0xMi/stamp-output/derived.qs c77d6b07a6f93833
...
```
    ...
    ## Warning: No primary key recorded for /tmp/Rtmppy0xMi/stamp-output/derived.qs.
    ## ℹ You can add one with `st_add_pk()`.

    ## ✔ Loaded [qs2] ← /tmp/Rtmppy0xMi/stamp-output/derived.qs
    ## ✔ Recorded primary key for /tmp/Rtmppy0xMi/stamp-output/derived.qs --> id
    ## ✔ stamp options updated
    ##   require_pk_on_load = "FALSE"
...
```
    ...
    ## ✔ Loaded [qs2] ←
    ## /tmp/Rtmppy0xMi/stamp-output/derived.qs
