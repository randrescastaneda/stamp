...
root <- file.path(
  tempdir(check = TRUE),
  paste0("stamp-smoke-", as.integer(runif(1, 1e6, 9e6)))
)
...
has_qs <- requireNamespace("qs2", quietly = TRUE) ||
...
if (has_qs) {
  ...
  ok(
    nrow(vtab2) == vcount_before + 1L,
    "content change did not increase version count"
  )
  vid2 <- st_latest(p_qs)
  ok(
    !identical(vid1, vid2),
    "latest version id did not change after content change"
  )
  ...
  ok(
    any(file.exists(file.path(vdir, c("sidecar.json", "sidecar.qs2")))),
...
if (requireNamespace("data.table", quietly = TRUE)) {
  ...
  same(
    df[order(df$id), ],
    df_c2[order(df_c2$id), ],
    "roundtrip mismatch for csv"
  )
} else {
...
if (has_qs) {
  ...
  ok(
    ...
    "st_read_sidecar did not return metadata list"
  )
...
ok(
  identical(st_opts("meta_format", .get = TRUE), "json"),
  "meta_format not set"
)
ok(
  identical(st_opts("default_format", .get = TRUE), "rds"),
  "default_format not set"
)
st_opts_reset()
now <- st_opts(.get = TRUE)
ok(
  identical(now$meta_format, "json"),
  "reset did not restore defaults (meta_format)"
)
ok(
  identical(now$default_format, "qs2"),
  "reset did not restore defaults (default_format)"
)
...
ok(
  st_changed_reason(p_chk, x1, function(z) z) == "no_change",
  "unexpected change reported"
)

ok(
  grepl("content", st_changed_reason(p_chk, x2, function(z) z)),
  "content change not detected"
)

ok(
  grepl("code", st_changed_reason(p_chk, x1, function(z) z + 1)),
  "code change not detected"
)

dec <- st_should_save(p_chk, x1, function(z) z)
ok(
  !dec$save && dec$reason == "no_change",
  "st_should_save() should recommend skip"
)
